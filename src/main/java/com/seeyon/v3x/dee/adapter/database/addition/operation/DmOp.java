 ...
 * @author zhangfb
 */
public class DmOp implements DatabaseOp {
    @Override
    public MultiValueMap generateSql(Element tableElement, Map<String, String> targetIds,
        ...
        String tableNameBackup = tableElement.getName();

        // 取得ID数组
        String[] idArray = getIdsByTableName(targetIds, tableName, ctx);

        String[] idArrayBackup = getIdsByTableName(targetIds, tableName, ctx);

        // 分割模式名表名
        String[] splitName = tableName.split("\\.");
        String type = splitName[0];
        String table = splitName[1];
       ...
       for(int i = 0 ; i < idArray.length ; i ++){
        ...
        if (idArray != null) {
            ...
            for (Element rowElement : tableElement.getChildren()) {
                ...
                boolean flag = isNew(tableNameBackup, idArrayBackup, rowElement, loader);
                if (flag) {     // 如果目标数据库没有值，拼接插入字符串
                    generateInsertSql(tableName, sqlMap, rowElement);
                } else {        // 如果有值，拼接update字符串
                    generateUpdateSql(tableName, idArrayBackup, sqlMap, rowElement);
                }
            }
        }
