...
import com.seeyon.v3x.dee.util.SimpleLazyDataLoader;

/**   
 *   
 *   @package：com.seeyon.v3x.dee.adapter.database.addition.operation.DeOp.java       
 *   @author    chenmeng <br/>    
 *   @create-time   2016年11月30日   上午11:22:38     
 **/
public class newDefaultOp implements DatabaseOp {

	@Override
	public MultiValueMap generateSql(Element tableElement,
			Map<String, String> targetIds, DbDataSource dataSource,
			TransformContext ctx) throws TransformException {
        ...
        if (idArray != null) {
        	SimpleLazyDataLoader loader = null;
			try {
				loader = generateDataLoader(tableName, idArray,  dataSource.getConnection());
				...
				throw new TransformException("jdbcWriter generateSql error",e);
			} finally{
				if(loader!=null)
					loader.close();
			}
     ...
     * @return 惰性加载器
     */
    private SimpleLazyDataLoader generateDataLoader(String tableName, String[] idArray, Connection conn) {
        ...
        sql.append(" 1=1 ");
       return new SimpleLazyDataLoader(conn, sql.toString());
    }
    ...
    private boolean isNew(String tableName, String[] idArray,
                          Element rowElement,SimpleLazyDataLoader loader) throws TransformException {
        boolean isNew = false;
